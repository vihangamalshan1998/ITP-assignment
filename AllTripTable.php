...
<head>
  <!-- Title -->
  <title>Administration | AllTripTable</title>
      ...
      <div class="container space-2">
        <div class="row">
            <div class="col-12">
                ...
                <h1 class="lead"><p style="color:#377dff;font-size:50px;text-align:center;font-weight: 900;"> ALL TRIPS </p></h1>
            </div>
        </div>
        <div class="card">
            ...
            <div class="table-responsive-md u-datatable">
              <table class="js-datatable table table-borderless u-datatable__striped u-datatable__content u-datatable__trigger mb-5"
                ...
                <tbody class="font-size-1">
                    <?php 
                        include ('../../config.php');
                        $sql= "SELECT * FROM trip WHERE Status != 'Finished'";
                        $results=$con->query($sql);
			            if ($results -> num_rows >0)
			            {
                            while($row = $results->fetch_assoc()){
                           echo '<tr>';
                              echo '<td class="align-middle text-primary">' .$row['TId'].'</td>';
                              echo '<td class="align-middle text-primary">'. $row['Deposit'].'</td>';
                              echo '<td class="align-middle text-primary">'. $row['Status'].'</td>';
                              echo '<td class="align-middle text-primary">'. $row['Description'].'</td>';
                              echo '<td><a href="finish_trip.php?id='.$row['TId'].'"><button type="button" class=" btn btn-sm btn-primary transition-3d-hover"> End Trip </button></a></td>';
                              echo '<td><p><a href="tripViewMore.php?id='.$row['TId'].'">more details>></a></p></td>';
                            echo '</tr>';                                
                        }} else {
                            echo "0 results";
                        }
